...
- **산출물(PDF) 서식**: `src/lib/pdf/` 에 KOSHA 공개 표준을 참고한 서식을 둔다. 양식의 **법적 정확성은
  출시 전 산업안전 전문가 검수가 필요**하며(코드 주석에 명시), 모든 산출물 하단에 `DOCUMENT_FOOTER_NOTE`
  ("적법성 보증 안 됨") 주석을 고정 노출한다. 한글 폰트는 `public/fonts`(나눔고딕, OFL) 임베드.
